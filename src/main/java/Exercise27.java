...
public class Exercise27 {
    public class SeparateChainingHashTableDoubleProbing<Key, Value> {
        ...
        public boolean contains(Key key) {
            if (key == null) {
                throw new IllegalArgumentException("Argument to Contains can not be null.");
            }
            return get(key) != null;
        }

        // M = 3; hash function: 11 k % M, 17 k % M
        int hash1(Key key) {
        ...
        public Value get(Key key) {
            if (key == null) {
                throw new IllegalArgumentException("Argument to get() can not be null");
            }
            int hash1 = hash1(key);
            int hash2 = hash2(key);
        ...
        public void delete(Key key) {
            ...
            int hash1 = hash1(key);
            int hash2 = hash2(key);
            if (!symbolTable[hash1].isEmpty() && (symbolTable[hash1].size <= symbolTable[hash2].size ||
                    symbolTable[hash2].isEmpty())) {
