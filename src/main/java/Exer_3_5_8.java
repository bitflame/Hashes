...
public class Exer_3_5_8 {
    private class LinearProbingHashTableDuplicateKeys<Key, Value> {
        ...
        private void resize(int newSize) {
            LinearProbingHashTableDuplicateKeys<Key, Value> tempHashTable =
                    new LinearProbingHashTableDuplicateKeys<>(newSize);
        ...
        public Value get(Key key) {
            if (key == null) throw new IllegalArgumentException("key to get() can not be null");
        ...
        public void put(Key key, Value value) {
            if (key == null) throw new IllegalArgumentException("key to put() cannot be null");
            if (value == null) {
                delete(key);
                return;
            }
        ...
        public void delete(Key key) {
            if (key == null) throw new IllegalArgumentException("key to delete() can not be null");
            if (!contains(key)) return;
            int tabaleIndex = hash(key);
            while (keys[tabaleIndex] != key) {
                tabaleIndex = tabaleIndex + 1 % size;
            }
            keys[tabaleIndex] = null;
            values[tabaleIndex] = null;
            keysSize--;
            tabaleIndex = tabaleIndex + 1 % size;
            while (keys[tabaleIndex] != null) {
                Key keyToRedo = keys[tabaleIndex];
                Value valueToRedo = values[tabaleIndex];
    ...
    public static void main(String[] args) {
        Exer_3_5_8 exercise8 = new Exer_3_5_8();
