...
public class Exercise40_PlotsLinearProbing {
    private class LinearProbingHashTableCost<Key, Value> extends LinearProbingHashTable<Key, Value> {
        ...
        public LinearProbingHashTableCost(int size) {
            super(size);
        }
    ...
    private static final String TALE_FILE_PATH = Constants.TALE_OF_TWO_CITIES_FILE;

    public static void main(String[] args) {
        //String[] wordsInTale = FileUtil.getAllStringsFromFile(TALE_FILE_PATH);
        int minLength = 8;
        //new Exercise40_PlotsLinearProbing().frequencyCouter(wordsInTale, minLength);
    }

    private String frequencyCouter(String[] words, int minLength) {
        String title = "Linear-probing hash table costs using put(0 in FrequencyCounter";
        ...
        for (String word : words) {
            ...
            if (!linearProbingHashTableCost.contains(word)) {
                ...
                cost = linearProbingHashTableCost.putAndComputeCost(word, linearProbingHashTableCost.get(word) + 1,
                        true);
            }
